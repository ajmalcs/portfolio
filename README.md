# ajmal.cs

**Cloud Data Professional | Technology Lead - Data & Analytics | BI Solutions Architect**

Cloud Data Professional with extensive experience in Business Intelligence, Data Analytics, and Engineering. Specialized in the US Logistics domain with a proven track record of delivering end-to-end BI solutions. Currently serving as a **Technology Lead** at Infosys in Ottawa, Canada.

---

## 🛠️ Technical Skills

| Category | Primary Skills | Secondary Skills |
|----------|----------------|------------------|
| **Cloud & Infrastructure** | Azure Data Factory, Synapse Analytics, Azure Databricks, Azure Key Vault, Microsoft Entra ID (RBAC) | Azure Functions, Logic Apps, Event Hubs, Azure Virtual Networks, Azure Policy, Cost Management |
| **Database & Storage** | ADLS Gen2, Delta Lake, SQL Server, Cosmos DB, MongoDB | Elastic Search, BigQuery |
| **BI & Visualization** | Power BI, Azure Analysis Services, Looker, LookML | Jasper Reports, Kibana |
| **Dev & Operations** | Python, Apache Spark, Azure DevOps (CI/CD), PowerShell | Docker, Kubernetes (AKS), Git, Logstash |

---

## 💼 Professional Experience

### **Infosys** | Technology Lead - Data & Analytics
*Ottawa, Canada | 2021 – Present*
*   Leading small teams and collaborating with offshore units.
*   Delivering end-to-end BI solutions with Azure and Power BI.
*   Architecting data pipelines and analytics platforms.
*   Driving complex logistics domain transformation projects.

### **Infosys** | Technology Analyst
*India | 2019 – 2021*
*   Developed BI dashboards and reporting solutions.
*   Implemented ETL pipelines using Azure Data Factory.
*   Collaborated on enterprise data warehouse projects.

### **Stabilix Solutions** | Software Engineer
*India | 2017 – 2019*
*   Java/Spring Boot web application development.
*   Search optimization and performance tuning.
*   Full-stack development and API design.

---

## 🏆 Certifications

**Expert & Specialized**
*   **Microsoft Certified: Azure Solutions Architect Expert** (Microsoft)
*   **Databricks Certified Generative AI Engineer Associate** (Databricks)
*   **Microsoft Certified: Azure Administrator Associate** (Microsoft)

**Professional**
*   Infosys Certified Power BI Advanced Developer
*   Microsoft Certified: Azure Data Fundamentals
*   Databricks Academy Accreditation - Generative AI Fundamentals
*   IBM Blockchain Essentials

---


